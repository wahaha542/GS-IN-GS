...
def replace_color_scale_rotation_info(low_contribution_vertices, ggbond_vertices, replacement_table):
    """将ggbond高斯球的颜色、缩放和旋转信息替换到低贡献高斯球中"""
    print("开始替换颜色、缩放和旋转信息...")
    ...
    for low_idx, ggbond_idx in replacement_table.items():
        # 更新颜色属性
        new_vertices[low_idx]['f_dc_0'] = ggbond_vertices[ggbond_idx]['f_dc_0']
        new_vertices[low_idx]['f_dc_1'] = ggbond_vertices[ggbond_idx]['f_dc_1']
        new_vertices[low_idx]['f_dc_2'] = ggbond_vertices[ggbond_idx]['f_dc_2']
    ...
    print("颜色、缩放和旋转信息替换完成")
    return new_vertices

def main():
    """主函数"""
    # 创建输出目录
    output_dir = r"D:\latest\new way\output\replace_csr2"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    # 文件路径
    low_contribution_path = r"D:\latest\new way\output\80000\low_contribution_spheres.ply"
    ggbond_path = r"D:\latest\data\ggbond\point_cloud\iteration_30000\point_cloud.ply"
    output_path = r"D:\latest\new way\output\replace_csr2\low_contribution_spheres_updated_color_scale_rotation.ply"
    replacement_table_path = r"D:\latest\new way\output\replace_csr2\replacement_table.json"
    ...
    # 验证低贡献模型的高斯球数量
    num_low_contribution = len(low_contribution_vertices)
    if num_low_contribution != 80000:
        print(f"警告: 低贡献模型包含 {num_low_contribution} 个高斯球，预期应为80000个")
        return
    ...
    # 替换颜色、缩放和旋转信息
    new_low_contribution_vertices = replace_color_scale_rotation_info(
    ...
    print(f"替换的高斯球数量: {num_ggbond}")
    print(f"未替换的高斯球数量: {num_low_contribution - num_ggbond}")
