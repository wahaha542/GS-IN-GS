...
def replace_color_opacity_scale_rotation_info(low_contribution_vertices, ggbond_vertices, replacement_table):
    """将ggbond高斯球的颜色、不透明度、缩放和旋转信息替换到低贡献高斯球中"""
    print("开始替换颜色、不透明度、缩放和旋转信息...")
    ...
    for low_idx, ggbond_idx in replacement_table.items():
        ...
        # 更新不透明度属性
        new_vertices[low_idx]['opacity'] = ggbond_vertices[ggbond_idx]['opacity']
...
def main():
    ...
    output_path = r"D:\latest\new way\output\replace_cαsr\low_contribution_spheres_updated_color_opacity_scale_rotation.ply"
    replacement_table_path = r"D:\latest\new way\output\replace_cαsr\replacement_table.json"
    ...
    # 替换颜色、不透明度、缩放和旋转信息
    new_low_contribution_vertices = replace_color_opacity_scale_rotation_info(
